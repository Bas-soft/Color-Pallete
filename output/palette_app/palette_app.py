...
def app():
    ...
    def show_color():
        numb_colors=int(select.get())

        for widget in color_frame.winfo_children():
            widget.destroy()

        for i in range(numb_colors):
            color=generate_color()
            color_label=customtkinter.CTkLabel(color_frame,fg_color=color,width=126,height=50,text='')
            color_label.grid(row=0,column=i,padx=5,pady=5)
            hex_label=customtkinter.CTkLabel(color_frame,text=color,width=10)
            hex_label.grid(row=1,column=i,pady=5,padx=5)
